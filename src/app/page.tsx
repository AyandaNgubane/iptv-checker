        ...
        {tab === 'player' && (
          <div className={styles.panel}>
            <div className={styles.pageHead}>
              ...
              <p className={styles.pageSub}>Play streams from Xtream Codes, M3U playlists, Stalker Portal, or paste a direct URL.</p>
            </div>
            ...
            <div className={styles.tabs}>
              {([['direct','ti-link','Direct URL'],['xtream','ti-code','Xtream'],['m3u','ti-list','M3U'],['stalker','ti-router','Stalker']] as [typeof playerMode, string, string][]).map(([m,ic,lb]) => (
                <button key={m} className={`${styles.tab} ${playerMode===m?styles.tabActive:''}`}
                  onClick={() => { setPlayerMode(m); setPlayerError(''); setPlayerLoaded(false); setPlayerActiveUrl('') }}>
            ...
            {playerMode === 'direct' && (
              <div className={styles.card}>
                ...
                <div className={styles.inputGroup}>
                  <label>Stream URL (M3U8, MP4, TS, HLS)</label>
                  <input type="text" value={playerUrl} onChange={e=>{setPlayerUrl(e.target.value);setPlayerError('');setPlayerLoaded(false)}}
                    placeholder="http://server:port/username/password/12345.m3u8"
                    className={styles.input}
                    onKeyDown={e=>{ if(e.key==='Enter'&&playerUrl.trim()){setPlayerActiveUrl(playerUrl.trim());setPlayerLoaded(true);setPlayerError('');setPlayerHistory(prev=>[{url:playerUrl.trim(),label:'Direct: '+playerUrl.trim().slice(-40)},...prev.filter(h=>h.url!==playerUrl.trim())].slice(0,20))} }}
                  />
                </div>
                ...
                <div className={styles.btnRow}>
                  <button className={styles.btnPrimary} onClick={()=>{
                    ...
                    setPlayerHistory(prev=>[{url:playerUrl.trim(),label:'Direct: '+playerUrl.trim().slice(-40)},...prev.filter(h=>h.url!==playerUrl.trim())].slice(0,20))
            ...
            {playerMode === 'xtream' && (
              <div className={styles.card}>
                <div className={styles.cardTitle}><i className="ti ti-code"/> Xtream Codes Connection</div>
                <div className={styles.row2}>
                ...
                <div className={styles.row2} style={{marginTop:12}}>
                  <div className={styles.inputGroup}><label>Password</label><input type="text" value={pxPass} onChange={e=>setPxPass(e.target.value)} placeholder="password" className={styles.input}/></div>
                  <div className={styles.inputGroup}><label>Search channels</label>
                    <input type="text" value={pxSearch} onChange={e=>setPxSearch(e.target.value)} placeholder="e.g. ESPN, SuperSport..." className={styles.input}/>
                  </div>
                </div>
                {playerError && <div className={styles.errorBox}><i className="ti ti-alert-circle"/> {playerError}</div>}
                <div className={styles.btnRow} style={{marginTop:14}}>
                  <button className={styles.btnPrimary} disabled={pxLoading} onClick={async()=>{
                    ...
                    setPxLoading(true);setPlayerError('')
                    try{
                      const s=pxServer.startsWith('http')?pxServer:`http://${pxServer}`
                      const res=await fetch(`/api/channels`,{method:'POST',headers:{'Content-Type':'application/json'},
                        body:JSON.stringify({type:'xtream',server:s,username:pxUser,password:pxPass,keyword:pxSearch||'.'})})
                      const data=await res.json()
                      if(!data.success)throw new Error(data.error||'Failed')
                      setPxChannels(data.channels.map((c:{name?:string;streamUrl?:string;logo?:string|null;category?:string})=>({name:c.name||'Unknown',url:c.streamUrl||'',logo:c.logo||null,category:c.category||''})))
                    }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Failed')}
                    finally{setPxLoading(false)}
                  }}>
                    {pxLoading?<><span className={styles.spinner}/> Loading...</>:<><i className="ti ti-search"/> Load Channels</>}
                  </button>
                  <button className={styles.btnSecondary} onClick={()=>setPxSearch('')}><i className="ti ti-x"/> Clear Search</button>
                </div>
              </div>
            )}
            ...
            {playerMode === 'm3u' && (
              <div className={styles.card}>
                ...
                <div className={styles.inputGroup}>
                  ...
                  <input type="text" value={m3uSearch} onChange={e=>setM3uSearch(e.target.value)} placeholder="Filter by channel name..." className={styles.input}/>
                </div>
                {playerError && <div className={styles.errorBox}><i className="ti ti-alert-circle"/> {playerError}</div>}
                <div className={styles.btnRow}>
                  <button className={styles.btnPrimary} disabled={m3uLoading} onClick={async()=>{
                    ...
                    setM3uLoading(true);setPlayerError('')
                    try{
            ...
            {playerMode === 'stalker' && (
              <div className={styles.card}>
                ...
                <div className={styles.inputGroup} style={{marginTop:12}}>
                  <label>Search channels</label>
                  <input type="text" value={spSearch} onChange={e=>setSpSearch(e.target.value)} placeholder="e.g. SuperSport, SABC..." className={styles.input}/>
                </div>
                {playerError && <div className={styles.errorBox}><i className="ti ti-alert-circle"/> {playerError}</div>}
                <div className={styles.btnRow} style={{marginTop:14}}>
                  <button className={styles.btnPrimary} disabled={spLoading} onClick={async()=>{
                    ...
                    setSpLoading(true);setPlayerError('')
                    try{
                      const res=await fetch('/api/channels',{method:'POST',headers:{'Content-Type':'application/json'},
                        body:JSON.stringify({type:'stalker',portalUrl:spPortal,mac:spMac,keyword:spSearch||'.'})})
                      const data=await res.json()
                      if(!data.success)throw new Error(data.error||'Failed')
                      setSpChannels(data.channels.map((c:{name?:string;cmd?:string;logo?:string|null;category?:string})=>({name:c.name||'Unknown',cmd:c.cmd||'',logo:c.logo||null,category:c.category||''})))
                    }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Failed')}
                    finally{setSpLoading(false)}
                  }}>
                    {spLoading?<><span className={styles.spinner}/> Loading...</>:<><i className="ti ti-search"/> Load Channels</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {/* ── Xtream channel list ── */}
            {playerMode==='xtream' && pxChannels.length>0 && (
              <div className={styles.card}>
                <div className={styles.cardTitle}><i className="ti ti-list"/> Channels ({pxChannels.filter(c=>!pxSearch||c.name.toLowerCase().includes(pxSearch.toLowerCase())).length})</div>
                <div className={styles.channelList}>
                  {pxChannels.filter(c=>!pxSearch||c.name.toLowerCase().includes(pxSearch.toLowerCase())).slice(0,100).map((ch,i)=>(
                    <div key={i} className={styles.channelRow}>
                      ...
                      <div className={styles.channelInfo}><div className={styles.channelName}>{ch.name}</div><div className={styles.channelMeta}>{ch.category}</div></div>
                      <button className={styles.btnSmall} onClick={()=>{setPlayerActiveUrl(ch.url);setPlayerLoaded(true);setPlayerError('');setPlayerHistory(prev=>[{url:ch.url,label:ch.name},...prev.filter(h=>h.url!==ch.url)].slice(0,20))}}>
            ...
            {/* ── M3U channel list ── */}
            {playerMode==='m3u' && m3uChannels.length>0 && (
              <div className={styles.card}>
                <div className={styles.cardTitle}><i className="ti ti-list"/> Playlist ({m3uChannels.filter(c=>!m3uSearch||c.name.toLowerCase().includes(m3uSearch.toLowerCase())).length} channels)</div>
                <div className={styles.channelList}>
                  {m3uChannels.filter(c=>!m3uSearch||c.name.toLowerCase().includes(m3uSearch.toLowerCase())).slice(0,100).map((ch,i)=>(
                    <div key={i} className={styles.channelRow}>
                      <div className={styles.channelIcon}>{ch.logo?<img src={ch.logo} alt="" style={{width:32,height:32,objectFit:'contain'}} onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>:<i className="ti ti-tv" style={{fontSize:18,color:'var(--accent3)'}}/>}</div>
                      <div className={styles.channelInfo}><div className={styles.channelName}>{ch.name}</div><div className={styles.channelMeta}>{ch.group}</div></div>
                      <button className={styles.btnSmall} onClick={()=>{setPlayerActiveUrl(ch.url);setPlayerLoaded(true);setPlayerError('');setPlayerHistory(prev=>[{url:ch.url,label:ch.name},...prev.filter(h=>h.url!==ch.url)].slice(0,20))}}>
                        <i className="ti ti-player-play"/> Play
                      </button>
                    </div>
            ...
            {playerMode==='stalker' && spChannels.length>0 && (
              <div className={styles.card}>
                <div className={styles.cardTitle}><i className="ti ti-list"/> Channels ({spChannels.filter(c=>!spSearch||c.name.toLowerCase().includes(spSearch.toLowerCase())).length})</div>
                <div className={styles.channelList}>
                  {spChannels.filter(c=>!spSearch||c.name.toLowerCase().includes(spSearch.toLowerCase())).slice(0,100).map((ch,i)=>(
                    <div key={i} className={styles.channelRow}>
                      <div className={styles.channelIcon}>{ch.logo?<img src={ch.logo} alt="" style={{width:32,height:32,objectFit:'contain'}} onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>:<i className="ti ti-tv" style={{fontSize:18,color:'var(--accent3)'}}/>}</div>
                      <div className={styles.channelInfo}><div className={styles.channelName}>{ch.name}</div><div className={styles.channelMeta}>{ch.category}</div></div>
                      <button className={styles.btnSmall} disabled={spLoading} onClick={async()=>{
                        setPlayerError('')
                        try{
                          const res=await fetch('/api/stalker-stream',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({portalUrl:spPortal,mac:spMac,cmd:ch.cmd})})
                          const data=await res.json()
                          if(data.error)throw new Error(data.error)
                        ...
                        }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Could not get stream')}
                      }}>
                        <i className="ti ti-player-play"/> Play
                      </button>
                    </div>
            ...
            <div className={styles.notice}>
              <i className="ti ti-info-circle"/>
              <div><strong>Supported:</strong> HLS (.m3u8), MP4, TS, direct HTTP streams, M3U playlists.&nbsp;
              <strong>Not supported in browser:</strong> RTMP (use VLC instead).&nbsp;
              <strong>Note:</strong> Some streams may be blocked by CORS — if it plays in VLC but not here, that&apos;s the cause.</div>
            </div>
          </div>
        )}
