...
export default function Home() {
  ...
  // Base64 state
  // Player state
  const [playerMode, setPlayerMode] = useState<'direct'|'xtream'|'m3u'|'stalker'>('direct')
      ...
      <main className={styles.main}>
        ...
        {tab === 'player' && (
          <div className={styles.panel}>
            ...
            <div className={styles.tabs}>
              {([['direct','ti-link','Direct URL'],['xtream','ti-code','Xtream'],['m3u','ti-list','M3U'],['stalker','ti-router','Stalker']] as [typeof playerMode,string,string][]).map(([m,ic,lb])=>(
              ...
              <div className={styles.card}>
                ...
                <div className={styles.inputGroup}>
                  ...
                  <input type="text" value={playerUrl}
                    onChange={e=>{setPlayerUrl(e.target.value);setPlayerError('');setPlayerLoaded(false)}}
                    placeholder="http://server:port/user/pass/12345.m3u8"
                    className={styles.input}
                    onKeyDown={e=>{if(e.key==='Enter'&&playerUrl.trim()){setPlayerActiveUrl(playerUrl.trim());setPlayerLoaded(true);setPlayerError('')}}}
                  />
                </div>
                {playerError&&<div className={styles.errorBox}><i className="ti ti-alert-circle"/> {playerError}</div>}
                <div className={styles.btnRow}>
                  <button className={styles.btnPrimary} onClick={()=>{
                    if(!playerUrl.trim()){setPlayerError('Enter a stream URL');return}
                    setPlayerActiveUrl(playerUrl.trim());setPlayerLoaded(true);setPlayerError('')
                    setPlayerHistory(prev=>[{url:playerUrl.trim(),label:playerUrl.trim().slice(0,60)},...prev.filter(h=>h.url!==playerUrl.trim())].slice(0,20))
                  }}><i className="ti ti-player-play"/> Play</button>
                ...
                <div className={styles.btnRow} style={{marginTop:14}}>
                  <button className={styles.btnPrimary} disabled={pxLoading} onClick={async()=>{
                    ...
                    try{
                      ...
                      const res=await fetch('/api/player-channels',{method:'POST',headers:{'Content-Type':'application/json'},
                        body:JSON.stringify({type:'xtream',server:s,username:pxUser,password:pxPass})})
                      const data=await res.json()
                      if(data.error)throw new Error(data.error)
                      setPxChannels(data.channels)
                      setPxCategories(data.categories||[])
                    }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Failed')}
                    finally{setPxLoading(false)}
                  }}>
                    {pxLoading?<><span className={styles.spinner}/> Loading all channels...</>:<><i className="ti ti-download"/> Load Channels</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {playerMode==='m3u' && (
              <div className={styles.card}>
                <div className={styles.cardTitle}><i className="ti ti-list"/> M3U Playlist URL</div>
                <div className={styles.inputGroup}>
                ...
                <div className={styles.btnRow}>
                  <button className={styles.btnPrimary} disabled={m3uLoading} onClick={async()=>{
                    ...
                    try{
                      ...
                      const cats=[...new Set((data.channels as {group:string}[]).map(c=>c.group).filter(Boolean))] as string[]
              ...
              <div className={styles.card}>
                ...
                <div className={styles.btnRow} style={{marginTop:14}}>
                  <button className={styles.btnPrimary} disabled={spLoading} onClick={async()=>{
                    ...
                    try{
                      const res=await fetch('/api/player-channels',{method:'POST',headers:{'Content-Type':'application/json'},
                        body:JSON.stringify({type:'stalker',portalUrl:spPortal,mac:spMac})})
                      const data=await res.json()
                      if(data.error)throw new Error(data.error)
                      setSpChannels(data.channels)
                      setSpCategories(data.categories||[])
                    }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Failed')}
                    finally{setSpLoading(false)}
                  }}>
                    {spLoading?<><span className={styles.spinner}/> Loading all channels...</>:<><i className="ti ti-download"/> Load Channels</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {/* ── Channel list (shared renderer) ── */}
            {(()=>{
              type Ch = {name:string;url?:string;cmd?:string;logo:string|null;category:string;categoryId?:string}
              let channels: Ch[] = []
              let search = ''
              let catFilter = 'all'
              let setCatFilter: (v:string)=>void = ()=>{}
              let categories: string[] = []

              if(playerMode==='xtream'&&pxChannels.length>0){
                channels=pxChannels as Ch[];search=pxSearch;catFilter=pxCatFilter;setCatFilter=setPxCatFilter;categories=pxCategories
              } else if(playerMode==='m3u'&&m3uChannels.length>0){
                channels=m3uChannels.map(c=>({...c,category:c.group})) as Ch[];search=m3uSearch;catFilter=m3uCatFilter;setCatFilter=setM3uCatFilter;categories=m3uCategories
              } else if(playerMode==='stalker'&&spChannels.length>0){
                channels=spChannels as Ch[];search=spSearch;catFilter=spCatFilter;setCatFilter=setSpCatFilter;categories=spCategories
              } else return null
              ...
              // Group by category
              const grouped: Record<string,Ch[]> = {}
              filtered.forEach(ch=>{
              ...
              const playChannel = async (ch: Ch) => {
                setPlayerError('')
                if(playerMode==='stalker'&&ch.cmd){
                  try{
                    const res=await fetch('/api/stalker-stream',{method:'POST',headers:{'Content-Type':'application/json'},
                      body:JSON.stringify({portalUrl:spPortal,mac:spMac,cmd:ch.cmd})})
                    const data=await res.json()
                    if(data.error)throw new Error(data.error)
                    setPlayerActiveUrl(data.url);setPlayerLoaded(true)
                    setPlayerHistory(prev=>[{url:data.url,label:ch.name},...prev.filter(h=>h.url!==data.url)].slice(0,20))
                  }catch(e:unknown){setPlayerError(e instanceof Error?e.message:'Could not resolve stream')}
                } else if(ch.url){
                  setPlayerActiveUrl(ch.url);setPlayerLoaded(true)
                  setPlayerHistory(prev=>[{url:ch.url!,label:ch.name},...prev.filter(h=>h.url!==ch.url)].slice(0,20))
                }
              }

              const totalVisible = Math.min(filtered.length, visibleCount)
              let shown = 0
              ...
              return (
                <div className={styles.card}>
                  ...
                  <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap'}}>
                    ...
                    <select
                      value={catFilter}
                      onChange={e=>{setCatFilter(e.target.value);setVisibleCount(50)}}
                      className={styles.selectSm}
                      style={{minWidth:140}}
                    >
                      <option value="all">All Categories ({filtered.length})</option>
                      {categories.sort().map(cat=>(
                        <option key={cat} value={cat}>{cat} ({channels.filter(c=>c.category===cat).length})</option>
                      ))}
                    </select>
                  </div>
                  ...
                  {Object.entries(grouped).map(([cat, chs])=>{
                    if(shown>=totalVisible) return null
                    const toShow = chs.slice(0, totalVisible-shown)
                    shown += toShow.length
                    return (
                      <div key={cat} style={{marginBottom:8}}>
                        <div style={{fontSize:11,fontWeight:700,letterSpacing:'0.06em',textTransform:'uppercase',color:'var(--text3)',padding:'8px 0 4px',borderBottom:'1px solid var(--border)',marginBottom:4}}>
                          <i className="ti ti-folder" style={{marginRight:6}}/>{cat} <span style={{color:'var(--accent3)'}}>({chs.length})</span>
                        </div>
                        {toShow.map((ch,i)=>(
                          <div key={i} className={styles.channelRow}>
                            <div className={styles.channelIcon}>
                            ...
                            <button className={styles.btnSmall} onClick={()=>playChannel(ch)}>
                  ...
                  {filtered.length===0&&(
                    <div className={styles.emptyState}><i className="ti ti-search-off" style={{fontSize:32,display:'block',marginBottom:8}}/><p>No channels match your search.</p></div>
                  )}
                </div>
              )
            })()}
                ...
                <div style={{display:'flex',flexDirection:'column',gap:4}}>
                  {playerHistory.map((h,i)=>(
                    <div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 10px',background:'var(--bg3)',borderRadius:'var(--radius)',border:'1px solid var(--border)'}}>
                      ...
                      <button onClick={()=>{setPlayerActiveUrl(h.url);setPlayerLoaded(true);setPlayerError('')}}
                        style={{background:'none',border:'none',color:'var(--accent3)',cursor:'pointer',fontSize:12,fontWeight:600,padding:'2px 8px',whiteSpace:'nowrap'}}>
                        <i className="ti ti-player-play"/> Play
                      </button>
                    </div>
                  ))}
            ...
            <div className={styles.notice}>
              <i className="ti ti-info-circle"/>
              <div><strong>Supported:</strong> HLS (.m3u8), MP4, TS, HTTP streams. &nbsp;<strong>Not supported in browser:</strong> RTMP. &nbsp;<strong>Note:</strong> Some streams block CORS — if it plays in VLC but not here, that&apos;s the cause.</div>
            </div>
          </div>
        )}
      ...
      <nav className={styles.bottomNav}>
        <div className={styles.bottomNavInner}>
          {([
            ...
            <button
              key={item.id}
              className={`${styles.bottomNavBtn} ${tab === item.id ? styles.bottomNavActive : ''}`}
