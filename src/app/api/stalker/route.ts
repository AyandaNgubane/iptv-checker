...
function parseExpiry(raw: unknown): { status: 'valid' | 'expired'; daysLeft: number | null; expiry: string } {
  if (!raw) return { status: 'valid', daysLeft: null, expiry: 'Unlimited' }
  const s = String(raw).trim()

  // Zero/empty dates mean "not set" on most portals — treat as unlimited
  if (!s || /^0000-00-00/.test(s) || s === '0') {
    return { status: 'valid', daysLeft: null, expiry: 'Unlimited' }
  }

  let expMs: number
  if (/^\d{10}$/.test(s)) expMs = parseInt(s) * 1000
  else if (/^\d{13}$/.test(s)) expMs = parseInt(s)
  else expMs = new Date(s).getTime()  // handles "June 13, 2027, 11:41 pm" and ISO strings
...
async function checkStalkerMac(portalUrl: string, mac: string) {
  ...
  // Some portals repurpose "phone" to store a human-readable expiry date (e.g. "June 13, 2027, 11:41 pm").
  // Prefer any field that actually parses to a real date over zero-dates / nulls.
  const expiryCandidates = [
    acc.phone,
    acc.end_date,
    acc.tariff_expired_date,
    acc.expire_billing_date,
    acc.subscription_end,
    profileJs.phone,
    profileJs.tariff_expire_date,
    profileJs.expire_billing_date,
    profileJs.end_date,
  ]
  const expiryRaw = expiryCandidates.find(v => {
    if (!v) return false
    const s = String(v).trim()
    return s && !/^0000-00-00/.test(s) && s !== '0'
  }) ?? null
  ...
  const timezone = (acc.timezone as string) ?? (profileJs.timezone as string) ?? (profileJs.default_timezone as string) ?? null
