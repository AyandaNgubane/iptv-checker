  ...
  useEffect(() => {
    ...
    const loadHLS = (src: string) => {
      ...
      import('hls.js').then(mod => {
        ...
        hls.on(Hls.Events.FRAG_LOADING, (_: unknown, d: { frag?: { url?: string; sn?: number | string } }) => {
          addLog(`Loading frag #${d.frag?.sn}: ${String(d.frag?.url||'').slice(0,80)}`, 'info')
        })
        hls.on(Hls.Events.FRAG_LOADED, (_: unknown, d: { stats?: { total?: number } }) => {
          addLog(`Frag loaded (${((d.stats?.total||0)/1024).toFixed(0)}kb)`, 'ok')
        })
