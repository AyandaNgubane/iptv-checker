...
type Tab = 'xtream' | 'stalker' | 'channels' | 'history'
    ...
    <div className={styles.app}>
      ...
      <header className={styles.header}>
        ...
        <nav className={styles.headerNav}>
          {(['xtream','stalker','channels','history'] as Tab[]).map(t => (
            <button key={t} className={`${styles.navBtn} ${tab === t ? styles.navBtnActive : ''}`} onClick={() => setTab(t)}>
              <i className={`ti ti-${t === 'xtream' ? 'code' : t === 'stalker' ? 'router' : t === 'channels' ? 'search' : 'history'}`} />
              {t === 'xtream' ? 'Xtream Codes' : t === 'stalker' ? 'Portal Stalker' : t === 'channels' ? 'Channel Finder' : 'History'}
            </button>
          ))}
        </nav>
      </header>
