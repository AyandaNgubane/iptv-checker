...
export default function IPTVPlayer({ url, onError, onLoad }: Props) {
  ...
  useEffect(() => {
    ...
    addLog(`Type: ${isM3U8?'M3U8':isTS?'TS':isMP4?'MP4':'unknown — treating as TS'}`)
    ...
    const loadHLS = (src: string) => {
      ...
      import('hls.js').then(mod => {
        ...
        hls.on(Hls.Events.FRAG_LOADED, () => addLog('Segment ✓', 'ok'))
        hls.on(Hls.Events.ERROR, (_: unknown, d: {
          fatal?: boolean; type?: string; details?: string; response?: { code?: number }
        }) => {
          const info = `${d.type}/${d.details}${d.response?.code?` HTTP${d.response.code}`:''}`
          addLog(`HLS ${d.fatal?'FATAL':'warn'}: ${info}`, d.fatal?'error':'info')
          if (d.fatal) {
            setStatus('error'); setStatusMsg(`Stream error: ${d.details||d.type}`)
    ...
    if (isM3U8) {
      ...
      // TS or unknown — wrap in inline M3U8, route the segment through the streaming proxy
      addLog('Wrapping in M3U8, routing segment through streaming proxy')
      const proxiedSegment = px(url)
      const m3u8 = [
        '#EXTM3U',
        '#EXT-X-VERSION:3',
